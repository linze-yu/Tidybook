...
## 绘图框架
```{r}
p框架 <- ggplot(data = q, aes(x = group1, y = CO2, color = group1, fill = group1))
p框架
```
...
```{r}
p美学外图层 <- p框架 +
  stat_boxplot(aes(x = group1, y = CO2), geom = "errorbar", color = 1)
p美学外图层
```
...
```{r}
p美学内图层 <- p美学外图层 +
  stat_boxplot(aes(x = group1, y = CO2, color = group1, fill = group1), geom = "boxplot", alpha = 0.5)
p美学内图层
```
...
```{r}
p其他包图层 <- p美学内图层 +
  ggbeeswarm::geom_beeswarm(cex = 4, priority = "density", size = 5, shape = 21, alpha = 0.3, stroke = 2, method = "swarm", corral = "gutter")
p其他包图层
```
...
```{r}
p刻线 <- p轴范围 +
  scale_y_continuous(breaks = seq(0, 200, by = 20), minor_breaks = seq(0, 200, by = 10))
p刻线
```
