...
# 200d
```{r}
import("D:/desktop/book/tidybook/data/200d/工作簿1.xlsx") %>% data.table() -> data
...
data %>%
  export("D:/desktop/book/tidybook/data/200d/省1.xlsx")
```

```{r}
import("D:/desktop/book/tidybook/data/200d/使用.xlsx") %>% data.table() -> data
...
```{r}
data %>%
  left_join_dt(shp2019[[2]], by = c("城市" = "市")) -> data1
```

```{r}
ggplot() +
  geom_sf(data = shp2019[[1]], fill = "NA", size = 0.1, color = "#96999C") +
  geom_sf(data = shp2019[[2]], fill = "NA", size = 0.1, color = "#96999C") +
  # geom_sf(data = shp2019[[3]], fill = "NA", size = 0.1, color = "#96999C") +
  geom_sf(data = shp2019[[4]], fill = "NA", size = 0.5, color = "#96999C") +
  geom_sf(data = data1, size = 0.1, color = "#96999C", aes(geometry = geometry, fill = 确诊)) + #
...
```{r}
q %>%
  ggplot(aes(x = CO2, y = O2, color = age)) +
  geom_text_repel(aes(label = pH), size = 6) +
  scale_color_gradientn(colors = viridis::viridis(500)) +
  expand_limits(fill = seq(65, 85, by = 5)) +
  Yuri_theme
```
