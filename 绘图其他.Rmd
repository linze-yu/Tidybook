...
data %>%
  slice_dt(1:200) %>%
  longer_dt(BC_VF, BC_CARD, RELATIONSHIP.0, COHAB, name = "group", value = "value") %>%
...
data %>%
  longer_dt(BC_VF, BC_CARD, RELATIONSHIP.0, COHAB, name = "group", value = "value") %>%
  ggplot(aes(x = group, y = value, color = COHAB, fill = COHAB)) +
...
```{r, echo = F}
data %>%
  longer_dt(RELATIONSHIP.0, COHAB, name = "group", value = "value") -> c
